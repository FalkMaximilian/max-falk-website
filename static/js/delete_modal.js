...
const lists = document.querySelectorAll('.todolist-list-item')

lists.forEach(function(elem) {
    elem.addEventListener('click', function() {
        // Get the listpk and load the tasks for this list
    })
})
        ...
        for (var i in list_list) {
            ...
            var item = `
                <li class="nav-item hstack rounded my-1 bg-body-tertiary todolist-list-item">
                    <a class="nav-link text-break fs-5 me-2" href="#" style="overflow-wrap: break-word;">${list_list[i].title}</a>
            ...
            if (active_list === null) {
                item = `
                    <li class="nav-item hstack rounded my-1 bg-primary todolist-list-item">
                        <a class="nav-link text-break fs-5 me-2 text-light" href="#" style="overflow-wrap: break-word;">${list_list[i].title}</a>
                        <a class="text-body-tertiary icon-link ms-auto me-2 fs-3" href="#">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16">
                                <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
                            </svg>
